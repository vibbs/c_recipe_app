...
class GlassDetails extends Component {
  constructor(props){
		super(props);
		this.state = {
      details : '',
  ...
  onDelete(){
    axios.delete(`http://localhost:3000/api/glasses/${this.props.match.params.name}`)
    .then(response => {
  ...
  render() {
    ...
    return (
      ...
      <div  className="row" >
      <br/>
    <Link className="btn grey col s2" to="/glasses">Back</Link>
    ...
    {/* <Link className="btn" to={`/glasses/edit/${this.state.details.name}`}>
    Edit</Link> */}
